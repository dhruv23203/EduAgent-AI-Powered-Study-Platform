...
export default function ProgressPage() {
  ...
  function load() { const id = studentIdFromStorage(); const cached = localStorage.getItem(`eduagent_progress_${id}`); if (cached) setProgress(JSON.parse(cached)); setLoading(true); getProgress(id).then((p) => { setProgress(p); localStorage.setItem(`eduagent_progress_${id}`, JSON.stringify(p)); setError(""); }).catch((e) => setError(e instanceof Error ? e.message : "Progress will appear after your first quiz.")).finally(() => setLoading(false)); }
  useEffect(load, []);
